...
def test_bake_with_custom_name(cookies):
    context = {"project_name": "app-awesome"}
...
def test_bake_and_run_precommit_hooks(cookies):
    with bake_in_temp_dir(cookies) as result:
        assert result.project_path.is_dir()
...
def test_bake_with_prefix_name_fail(cookies):
    context = {"project_name": "mydata_awesome"}
    with bake_in_temp_dir(cookies, extra_context=context) as result:
        assert isinstance(result.exception, FailedHookException)


def test_bake_with_hyphen_slug_fail(cookies):
    context = {"project_slug": "dataproject-awesome"}
    with bake_in_temp_dir(cookies, extra_context=context) as result:
        assert isinstance(result.exception, FailedHookException)
